...
- La monetizzazione parte da tre piani mensili configurabili (`Starter`, `Creator`, `Studio`) con token inclusi. I piani usano un Price Stripe mensile salvato come `stripe_price_id`; il comando esplicito di sync crea i prodotti/prezzi nell’ambiente Stripe configurato. Una variazione locale di prezzo o valuta invalida l’ID e richiede una nuova sincronizzazione, perché i Price Stripe sono immutabili. Il checkout cliente crea solo il primo abbonamento; cambio piano e disdetta richiesta dal cliente restano da collegare. I token extra restano acquistabili separatamente.
