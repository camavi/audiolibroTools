...
Per tutta la dashboard, JSswift e' il framework ufficiale: JS, layout, componenti UI, token CSS e temi devono partire da JSswift. Nel JS della dashboard usare l'API tramite alias `_.` (`_.Layout`, `_.Icon`, `_.mount`, helper DOM, ecc.). CSS nativo del progetto e' ammesso solo per composizione specifica, branding Audiobook Tools o casi non coperti da JSswift.
...
- `resources/js/dashboard.js` deve restare la shell principale: importa `jsswift`, monta `#dashboard-root`, crea `_.Layout`, gestisce drawer, header, outlet e routing con `_.router`.
...
- Usare JSswift tramite alias globale `_` per layout, form, card, dialog, grid, input, select, button, alert, router, mount e stato reattivo con `_.rod`.
- Non creare CSS custom per la dashboard se JSswift copre il caso. Classi custom o CSS di progetto sono ammessi solo per composizione specifica, branding Audiobook Tools o casi non coperti dai componenti/token JSswift.
- Per chiamate backend usare `_.http.getJSON`, `_.http.postJSON` e gli helper HTTP JSswift; evitare `fetch` diretto salvo motivo tecnico documentato nel codice.
...
## JSswift: focus e controlli reattivi

Quando lavori con JSswift, evita che un parent dinamico si sottoscriva ai model dei controlli figli. Non leggere `.value` di un rod/model dentro la stessa funzione dinamica che crea `_.Input`, `_.Textarea`, `_.Select` o altri controlli editabili: a ogni modifica il parent verrebbe ricreato e il controllo perderebbe il focus.
...
- Non chiamare side effect (`load...`, `fetch`, `save`) dentro un render dinamico. Se indispensabile per retrocompatibilità, isolarli con `JSswift.reactive.untracked(() => ...)`.
- Anche l'idratazione iniziale dei model nei componenti JSswift deve usare `JSswift.reactive.untracked()`.
...
- Frontend: Node/Vite.
- Dashboard: sviluppata con JSswift come framework ufficiale.
