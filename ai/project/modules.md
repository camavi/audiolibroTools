...
- Calcolatore valore token: ogni riga del catalogo admin ha una dialog che confronta esclusivamente quell'addebito AI con i piani mensili e i pacchetti token attivi. Permette di simulare quantità di input/output per LLM (gli addebiti sono per 1K token) o minuti audio; usa i valori presenti nei campi, anche se non ancora salvati, e mostra dettaglio, totale commerciale e numero di simulazioni incluse. Per le immagini calcola sempre una generazione, accetta token cliente e costo GPT corrente in EUR modificabili solo nella simulazione e mostra valore del token per piano, costo e margine lordo stimato.
