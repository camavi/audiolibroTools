...
- Abbonamenti mensili: catalogo amministrativo iniziale di tre piani (`Starter`, `Creator`, `Studio`) con prezzo EUR e token inclusi al mese. Il portale cliente mostra piano corrente, rinnovo, token mensili, piani alternativi e azioni di cambio/disdetta; queste ultime restano disabilitate fino al checkout Stripe per abbonamenti. Rinnovi e allocazione mensile token restano fasi separate.
