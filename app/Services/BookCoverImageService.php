...
class BookCoverImageService
{
     ...
     */
    public function generate(Book $book, string $prompt, ImageCreditService $credits): BookDesignAsset
    {
        ...
        $pricing = AiModelPrice::query()
            ->where(['provider_key' => 'at-openai', 'model' => $model, 'modality' => 'image'])
            ->first();

        if ($pricing && ! $pricing->is_enabled) {
            $this->fail('Image generation is temporarily unavailable.');
        }
        $charge = (int) ($pricing?->customer_credits ?? 70);
        $reference = (string) Str::uuid();
        $credits->reserve($book, $charge, $reference);
        ...
        try {
            ...
            $credits->consume($book, $charge, $reference);

            return $asset;
        } catch (\Throwable $exception) {
            $credits->release($book, $charge, $reference);
            throw $exception;
        }
    }
