...
{
    public function render(Book $book, array $settings, ?iterable $sourceBlocks = null): string
    {
        $options = new Options;
        ...
        $pdf->loadHtml($this->html($book, $settings, $sourceBlocks), 'UTF-8');
        $pdf->render();
    ...
    public function store(Book $book, array $settings, ?int $publicationVersion = null, ?iterable $sourceBlocks = null): string
    {
        $versionPath = $publicationVersion ? "/v{$publicationVersion}" : '';
        $path = 'book-pdfs/'.$book->key_book.$versionPath.'/'.Str::slug($settings['metadata']['title'] ?: $book->name).'.pdf';
        Storage::disk('public')->put($path, $this->render($book, $settings, $sourceBlocks));

        return $path;
    }

    private function html(Book $book, array $settings, ?iterable $sourceBlocks = null): string
    {
