...
function openReimportDialog(keyBook) {
    ...
    const upload = _.Upload({ label: 'Updated manuscript', subtitle: 'DOCX, TXT or text-based PDF · max 25 MB', accept: '.docx,.txt,application/pdf', multiple: false, maxFileSize: 25 * 1024 * 1024, url: `/dashboard/api/books/${encodeURIComponent(keyBook)}/reimport-preview`, fieldName: 'manuscript', uploadButton: false, headers: { Accept: 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content || '' }, onStart: () => { loading.value = true; note.value = null; }, onSuccess: (item, { response }) => { const data = JSON.parse(response.text || '{}').data || null; JSswift.reactive.untracked(() => { if (data) data.selectionModels = Object.fromEntries((data.items || []).filter((change) => change.kind !== 'unchanged').map((change) => [change.selection_key, _.rod(change.kind !== 'removed')])); preview.value = data; }); }, onError: (item, { error }) => { note.value = { type: 'danger', message: error.message || 'Unable to compare this manuscript.' }; }, onFinish: () => { loading.value = false; } });
